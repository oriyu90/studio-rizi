...
- プロジェクトカードは常に正方形です。縦横比に応じた最小辺長で多列配置し、599px以下は2列。初期表示は縦長で3行、正方形に近い画面で2行、横長で1行です。名称は14px以上、対応OSは11px以上を保ち、小型カードでは説明を省略します（リスト／詳細で全文を読めます）。
...
- `npm test` は公開ファイルの検査と27件のUI・テーマ・SNSテストを実行します。今回の監査結果・検証範囲と制限は `UI-AUDIT-2026-08-27.md` に記録しています。
