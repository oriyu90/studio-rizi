...
for (const page of pageDefinitions) {
  for (const locale of localeDefinitions) {
    ...
    if (page.name === 'home') {
      const toggles = html.match(/<button\b[^>]*id="project-view-toggle"[^>]*>/g) || [];
      if (toggles.length !== 1 || !toggles[0].includes('data-next-view="list"') || !toggles[0].includes('aria-controls="project-grid"') || !toggles[0].includes('aria-label="')) {
        failures.push(`${page.name}/${locale.locale}: single accessible next-view toggle is missing`);
      }
      for (const view of ['cards','list']) if (!html.includes(`data-view-icon="${view}"`)) failures.push(`${page.name}/${locale.locale}: ${view} icon is missing`);
